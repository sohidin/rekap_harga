...
function renderPeriod(){const d=sortedPeriodData();$('periodCommodities').textContent=fmtInt(unique(d,'commodity').length);$('periodMarkets').textContent=fmtInt(unique(d,'market').length);$('periodRespondents').textContent=fmtInt(unique(d,'respondent').length);$('periodExtreme').textContent=fmtInt(d.filter(x=>Math.abs(x.change||0)>=20).length);if($('periodLegend'))$('periodLegend').innerHTML=periodLegendHtml();const p=paginate(d,state.periodPage,state.periodPageSize);state.periodPage=p.page;$('periodTableBody').innerHTML=p.rows.map(x=>rowHtml(x,false)).join('')||'<tr><td colspan="11">Tidak ada data sesuai filter.</td></tr>';$('periodPageInfo').textContent=state.periodPageSize==='all'?`Menampilkan semua • ${fmtInt(d.length)} data`:`Halaman ${p.page} dari ${p.pages} • ${fmtInt(d.length)} data`;bindSaveButtons();updatePeriodSortMarks()}
