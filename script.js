...
const state={all:[],filtered:[],view:'dashboard',periodPage:1,trendPage:1,periodPageSize:50,trendPageSize:50,periodSortKey:'rawOrder',periodSortDir:'asc',trendSortKey:'rawOrder',trendSortDir:'asc',matrixSortKey:'rawOrder',matrixSortDir:'asc',charts:{}};
...
function applyFilters(){const p=currentPeriod(),kab=$('kabFilter').value,kom=norm($('commodityFilter').value).toLowerCase(),qual=norm($('qualityFilter').value).toLowerCase(),mt=$('marketTypeFilter').value,q=norm($('searchFilter').value).toLowerCase(),extreme=$('extremeOnly').checked;state.filtered=state.all.filter(x=>(p==='Semua'||x.sheet===p)&&(!kab||x.kab===kab)&&(!kom||x.commodity.toLowerCase().includes(kom)||x.commodityCode.toLowerCase().includes(kom))&&(!qual||x.quality.toLowerCase().includes(qual))&&(!mt||x.marketType===mt)&&(!q||[x.kab,x.commodityCode,x.commodity,x.quality,x.market,x.respondent,x.note,x.period].join(' ').toLowerCase().includes(q))&&(!extreme||(Number.isFinite(x.change)&&Math.abs(x.change)>=20)));state.periodPage=1;state.trendPage=1;renderAll()}
...
function renderAll(){renderHeader();renderDashboard();renderPeriod();renderCommodity();renderMarket();renderTrend();renderEvaluation();renderMatrix()}
...
function trendBaseData(){
  const p=currentPeriod(),kab=$('kabFilter').value,kom=norm($('commodityFilter').value).toLowerCase(),qual=norm($('qualityFilter').value).toLowerCase(),mt=$('marketTypeFilter').value,q=norm($('searchFilter').value).toLowerCase(),cat=$('trendCategoryFilter')?.value||'';
  return state.all.filter(x=>(p==='Semua'||x.sheet===p)&&(!kab||x.kab===kab)&&(!kom||x.commodity.toLowerCase().includes(kom)||x.commodityCode.toLowerCase().includes(kom))&&(!qual||x.quality.toLowerCase().includes(qual))&&(!mt||x.marketType===mt)&&(!q||[x.kab,x.commodityCode,x.commodity,x.quality,x.market,x.respondent,x.note,x.period].join(' ').toLowerCase().includes(q))&&x.series.some(s=>s.valid)&&(!cat||x.series.some(s=>s.valid&&s.category?.id===cat)));
}
