...
const RH_SHEET={name:'RH web',lastColumn:'H',kabIndex:0,commodityCodeIndex:1,commodityIndex:2,qualityIndex:3,prevIndex:4,currentIndex:5,rhIndex:6,noteIndex:7,noteColumn:'H'};
const RH_KABS=['1902','1903','1906','1971'];
const state={all:[],filtered:[],rhAll:[],view:'dashboard',periodPage:1,trendPage:1,rhPage:1,rhPricePage:1,periodPageSize:50,trendPageSize:50,rhPageSize:50,rhPricePageSize:50,periodSortKey:'rawOrder',periodSortDir:'asc',trendSortKey:'rawOrder',trendSortDir:'asc',matrixSortKey:'rawOrder',matrixSortDir:'asc',rhSortKey:'rawOrder',rhSortDir:'asc',rhPriceSortKey:'rawOrder',rhPriceSortDir:'asc',charts:{}};
...
function populateDatalist(inputId,listId,items){const input=$(inputId),list=$(listId);if(!input||!list)return;list.innerHTML=items.map(v=>`<option value="${esc(v)}"></option>`).join('')}
...
function renderCommodity(){
  ...
  const quality=(qualityQuery?qualities.find(q=>q.toLowerCase().includes(qualityQuery)):qualities[0])||'';
  ...
  $('commoditySelectionInfo').innerHTML=focus?`<strong>${esc(code)}</strong> • ${esc(focus)} <span>×</span> <strong>${esc(quality||'Belum ada kualitas')}</strong> <small>• ${fmtInt(new Set(d.map(x=>x.kab).filter(Boolean)).size)} wilayah • gunakan filter di atas untuk mengganti komoditas/kualitas</small>`:'Tidak ada data sesuai filter.';
...
function trendBaseData(){
  const p=currentPeriod(),kab=$('kabFilter').value,kom=norm($('commodityFilter').value).toLowerCase(),qual=norm($('qualityFilter').value).toLowerCase(),mt=$('marketTypeFilter').value,q=norm($('searchFilter').value).toLowerCase(),cat=$('trendCategoryFilter')?.value||'',bands=selectedChangeBands();
  return state.all.filter(x=>(p==='Semua'||x.sheet===p)&&(!kab||x.kab===kab)&&(!kom||x.commodity.toLowerCase().includes(kom)||x.commodityCode.toLowerCase().includes(kom))&&(!qual||x.quality.toLowerCase().includes(qual))&&(!mt||x.marketType===mt)&&(!q||[x.kab,x.commodityCode,x.commodity,x.quality,x.market,x.respondent,x.note,x.period].join(' ').toLowerCase().includes(q))&&x.series.some(s=>s.valid)&&(!bands.length||x.series.some(s=>s.valid&&changeMatchesBands(s.change,bands)))&&(!cat||x.series.some(s=>s.valid&&s.category?.id===cat)));
}
...
function populateRhFilters(){
  populateDatalist('rhCommodityFilter','rhCommodityFilterList',unique(state.rhAll,'commodity'));
  populateDatalist('rhQualityFilter','rhQualityFilterList',unique(state.rhAll,'quality'));
}
function rhFilterValues(){return{kab:$('rhKabFilter')?.value||'',kom:norm($('rhCommodityFilter')?.value).toLowerCase(),qual:norm($('rhQualityFilter')?.value).toLowerCase(),cond:$('rhConditionFilter')?.value||'',q:norm($('rhSearchFilter')?.value).toLowerCase()}}
function rhBaseRows(){const f=rhFilterValues();return state.rhAll.filter(x=>(!f.kab||x.kab===f.kab)&&(!f.kom||x.commodity.toLowerCase().includes(f.kom)||x.commodityCode.toLowerCase().includes(f.kom))&&(!f.qual||x.quality.toLowerCase().includes(f.qual))&&(!f.q||[x.kab,x.commodityCode,x.commodity,x.quality,x.note].join(' ').toLowerCase().includes(f.q)))}
function rhPivotRows(){
  const f=rhFilterValues(),base=state.rhAll.filter(x=>(!f.kom||x.commodity.toLowerCase().includes(f.kom)||x.commodityCode.toLowerCase().includes(f.kom))&&(!f.qual||x.quality.toLowerCase().includes(f.qual))&&(!f.q||[x.commodityCode,x.commodity,x.quality,x.note].join(' ').toLowerCase().includes(f.q)));
  const map=new Map();base.forEach(x=>{const key=x.commodityCode||x.commodity;if(!key)return;if(!map.has(key))map.set(key,{rawOrder:x.rawOrder,commodityCode:x.commodityCode,commodity:x.commodity,quality:x.quality,byKab:{}});const p=map.get(key);p.rawOrder=Math.min(p.rawOrder,x.rawOrder);p.byKab[x.kab]=x});
  let rows=[...map.values()].map(r=>{const vals=RH_KABS.map(k=>r.byKab[k]?.rh).filter(Number.isFinite),up=vals.filter(v=>v>100).length,down=vals.filter(v=>v<100).length,eq=vals.filter(v=>Math.abs(v-100)<1e-9).length;let outlierKab='';if(up===1&&down>=2)outlierKab=RH_KABS.find(k=>r.byKab[k]?.rh>100)||'';if(down===1&&up>=2)outlierKab=RH_KABS.find(k=>r.byKab[k]?.rh<100)||'';return{...r,up,down,eq,outlierKab,mixed:up>0&&down>0}});
...
function rhValueClass(v,isOutlier){if(isOutlier)return'rh-value-outlier';if(!Number.isFinite(v))return'rh-value-missing';if(Math.abs(v-100)<1e-9)return'rh-value-neutral';return v>100?'rh-value-up':'rh-value-down'}
...
function renderRh(){if(!$('rhTableBody'))return;const d=sortedRhPivot(),allVals=d.flatMap(r=>RH_KABS.map(k=>r.byKab[k]?.rh).filter(Number.isFinite));$('rhKpiCommodity').textContent=fmtInt(d.length);$('rhKpiUp').textContent=fmtInt(allVals.filter(v=>v>100).length);$('rhKpiDown').textContent=fmtInt(allVals.filter(v=>v<100).length);$('rhKpiOutlier').textContent=fmtInt(d.filter(r=>r.outlierKab).length);const p=paginate(d,state.rhPage,state.rhPageSize);state.rhPage=p.page;$('rhTableBody').innerHTML=p.rows.map((r,i)=>`<tr><td>${state.rhPageSize==='all'?i+1:(p.page-1)*Number(state.rhPageSize)+i+1}</td><td><span class="commodity-code">${esc(r.commodityCode)}</span></td><td><strong>${esc(r.commodity)}</strong>${r.quality?`<small class="rh-quality">${esc(r.quality)}</small>`:''}</td>${RH_KABS.map(k=>`<td class="num">${rhCellHtml(r.byKab[k],r.outlierKab===k)}</td>`).join('')}</tr>`).join('')||'<tr><td colspan="7">Tidak ada data RH sesuai filter.</td></tr>';$('rhPageInfo').textContent=state.rhPageSize==='all'?`Menampilkan semua • ${fmtInt(d.length)} komoditas`:`Halaman ${p.page} dari ${p.pages} • ${fmtInt(d.length)} komoditas`;bindSaveButtons();updateRhSortMarks()}
function rhPriceRows(){const f=rhFilterValues();let rows=state.rhAll.filter(x=>(!f.kab||x.kab===f.kab)&&(!f.kom||x.commodity.toLowerCase().includes(f.kom)||x.commodityCode.toLowerCase().includes(f.kom))&&(!f.qual||x.quality.toLowerCase().includes(f.qual))&&(!f.q||[x.kab,x.commodityCode,x.commodity,x.quality,x.note].join(' ').toLowerCase().includes(f.q)));if(f.cond){const allowed=new Set(rhPivotRows().map(r=>r.commodityCode||r.commodity));rows=rows.filter(x=>allowed.has(x.commodityCode||x.commodity))}return rows}
function sortedRhPrice(){const arr=rhPriceRows(),k=state.rhPriceSortKey,d=state.rhPriceSortDir==='asc'?1:-1;return arr.sort((a,b)=>{let av=a[k],bv=b[k];if(typeof av==='number'||typeof bv==='number'){av=Number.isFinite(av)?av:-Infinity;bv=Number.isFinite(bv)?bv:-Infinity;return(av-bv)*d}return String(av??'').localeCompare(String(bv??''),'id')*d})}
function rhPriceRowHtml(x,n){const cls=rhValueClass(x.rh,false),chg=Number.isFinite(x.rh)?x.rh-100:null;return`<tr><td>${n}</td><td>${esc(x.kab)}</td><td><span class="commodity-code">${esc(x.commodityCode)}</span></td><td><strong>${esc(x.commodity)}</strong></td><td><span class="quality-text">${esc(x.quality)}</span></td><td class="num"><div class="period-price-card base-price"><span>Previous</span><strong>${fmtMoney(x.prev)}</strong></div></td><td class="num"><div class="period-price-card current-price ${chg>0?'cat-up-20':chg<0?'cat-down-20':'cat-stable'}"><span>Current</span><strong>${fmtMoney(x.current)}</strong></div></td><td class="num"><span class="rh-price-badge ${cls}">${Number.isFinite(x.rh)?x.rh.toFixed(2).replace('.',','):'–'}<small>${Number.isFinite(chg)?fmtPct(chg):'–'}</small></span></td><td>${noteCell(x)}</td><td>${saveButton(x)}</td></tr>`}
function renderRhPrice(){if(!$('rhPriceTableBody'))return;const d=sortedRhPrice();$('rhPriceKpiObs').textContent=fmtInt(d.length);$('rhPriceKpiUp').textContent=fmtInt(d.filter(x=>x.rh>100).length);$('rhPriceKpiDown').textContent=fmtInt(d.filter(x=>x.rh<100).length);$('rhPriceKpiStable').textContent=fmtInt(d.filter(x=>Number.isFinite(x.rh)&&Math.abs(x.rh-100)<1e-9).length);const p=paginate(d,state.rhPricePage,state.rhPricePageSize);state.rhPricePage=p.page;$('rhPriceTableBody').innerHTML=p.rows.map((x,i)=>rhPriceRowHtml(x,state.rhPricePageSize==='all'?i+1:(p.page-1)*Number(state.rhPricePageSize)+i+1)).join('')||'<tr><td colspan="10">Tidak ada data sesuai filter.</td></tr>';$('rhPricePageInfo').textContent=state.rhPricePageSize==='all'?`Menampilkan semua • ${fmtInt(d.length)} observasi`:`Halaman ${p.page} dari ${p.pages} • ${fmtInt(d.length)} observasi`;bindSaveButtons();updateRhPriceSortMarks()}
...
function rhPriceExportMatrix(){return[['No','Wilayah','Kode Komoditas','Komoditas','Kualitas','Previous','Current','RH','Keterangan'],...sortedRhPrice().map((x,i)=>[i+1,x.kab,x.commodityCode,x.commodity,x.quality,x.prev??'',x.current??'',x.rh??'',x.note])]}
