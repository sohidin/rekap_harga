...
const SHEETS={
  'Mingguan':{lastColumn:'Z',prevIndex:21,currentIndex:22,noteIndex:24,respondentIndex:10,noteColumn:'Y'},
  'Dwi Mingguan':{lastColumn:'Y',prevIndex:15,currentIndex:16,noteIndex:18,respondentIndex:10,noteColumn:'S'},
  'bulanan':{lastColumn:'U',prevIndex:11,currentIndex:12,noteIndex:14,respondentIndex:10,noteColumn:'O'}
};
const state={all:[],filtered:[],view:'dashboard',page:1,periodPage:1,sortKey:'change',sortDir:'desc',charts:{}};
...
function applyFilters(){const p=currentPeriod(),kab=$('kabFilter').value,kom=$('commodityFilter').value,mt=$('marketTypeFilter').value,q=norm($('searchFilter').value).toLowerCase(),extreme=$('extremeOnly').checked;state.filtered=state.all.filter(x=>(p==='Semua'||x.sheet===p)&&(!kab||x.kab===kab)&&(!kom||x.commodity===kom)&&(!mt||x.marketType===mt)&&(!q||[x.kab,x.commodity,x.quality,x.market,x.respondent,x.note,x.period].join(' ').toLowerCase().includes(q))&&(!extreme||(Number.isFinite(x.change)&&Math.abs(x.change)>=20)));state.page=1;state.periodPage=1;renderAll()}
...
function renderAll(){renderHeader();renderDashboard();renderPeriod();renderCommodity();renderMarket();renderEvaluation();renderRecap();updateSortMarks()}
...
function rowHtml(x,recap=false){return `<tr class="${Math.abs(x.change||0)>=20?'extreme-row':''}"><td>${esc(x.period)}</td><td>${esc(x.kab)}</td><td>${esc(x.commodity)}</td><td>${esc(x.quality)}</td>${recap?`<td>${esc(x.marketType)}</td>`:''}<td>${esc(x.market)}</td><td>${esc(x.respondent)}</td><td class="num">${fmtMoney(x.prev)}</td><td class="num">${fmtMoney(x.current)}</td><td class="num"><span class="pill ${x.change>0?'up':x.change<0?'down':'stable'} extreme">${fmtPct(x.change)}</span></td><td>${noteCell(x)}</td><td>${saveButton(x)}</td></tr>`}
