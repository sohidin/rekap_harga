...
const state={all:[],filtered:[],view:'dashboard',page:1,periodPage:1,trendPage:1,periodPageSize:50,trendPageSize:50,sortKey:'change',sortDir:'desc',trendSortKey:'maxAbsChange',trendSortDir:'desc',charts:{}};
...
function populateFilters(){populateSelect('kabFilter',unique(state.all,'kab'),'Semua wilayah');populateSelect('commodityFilter',unique(state.all,'commodity'),'Semua komoditas');populateSelect('marketTypeFilter',unique(state.all,'marketType'),'Semua jenis pasar');populateSelect('commodityFocus',unique(state.all,'commodity'),'Pilih komoditas')}
function applyFilters(){const p=currentPeriod(),kab=$('kabFilter').value,kom=$('commodityFilter').value,mt=$('marketTypeFilter').value,q=norm($('searchFilter').value).toLowerCase(),extreme=$('extremeOnly').checked;state.filtered=state.all.filter(x=>(p==='Semua'||x.sheet===p)&&(!kab||x.kab===kab)&&(!kom||x.commodity===kom)&&(!mt||x.marketType===mt)&&(!q||[x.kab,x.commodity,x.quality,x.market,x.respondent,x.note,x.period].join(' ').toLowerCase().includes(q))&&(!extreme||(Number.isFinite(x.change)&&Math.abs(x.change)>=20)));state.page=1;state.periodPage=1;state.trendPage=1;renderAll()}
...
function renderPeriod(){const d=sortedData();$('periodCommodities').textContent=fmtInt(unique(d,'commodity').length);$('periodMarkets').textContent=fmtInt(unique(d,'market').length);$('periodRespondents').textContent=fmtInt(unique(d,'respondent').length);$('periodExtreme').textContent=fmtInt(d.filter(x=>Math.abs(x.change||0)>=20).length);if($('periodLegend'))$('periodLegend').innerHTML=periodLegendHtml();const p=paginate(d,state.periodPage,state.periodPageSize);state.periodPage=p.page;$('periodTableBody').innerHTML=p.rows.map(x=>rowHtml(x,false)).join('')||'<tr><td colspan="11">Tidak ada data sesuai filter.</td></tr>';$('periodPageInfo').textContent=state.periodPageSize==='all'?`Menampilkan semua • ${fmtInt(d.length)} data`:`Halaman ${p.page} dari ${p.pages} • ${fmtInt(d.length)} data`;bindSaveButtons()}
...
function priceTimelineHtml(x){
  ...
  let html=`<div class="price-visual"><div class="spark-wrap">${sparklineSvg(points)}</div><div class="price-timeline">`;
  points.forEach((p,i)=>{
